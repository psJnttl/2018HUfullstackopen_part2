        ...
        personService.replace(numberOnly, dupeCheck.id).then ( (response) => {
          ...
          personService.addOne(newPerson).then( (response) => {
            const persons = this.state.persons.filter((person) => {
              return response.name.toLowerCase() !== person.name.toLowerCase();
            });
            const personAdded = persons.concat(response);
            this.setState({persons: personAdded, newName: '', newNumber: '',
            note: "Henkilö " + newPerson.name + " oli poistettu palvelimelta." +
                  "Lisätty nyt takaisin numerolla " + newPerson.number, noteStyle: 'oknote'});
